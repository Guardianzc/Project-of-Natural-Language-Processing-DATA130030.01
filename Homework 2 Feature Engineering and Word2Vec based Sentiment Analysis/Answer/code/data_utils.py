...
class StanfordSentiment:
    ...
    def tokens(self):
        #count the tokens, define their freqency and index
    ...
    def allSentences(self):
        # Return frequent words in each sentence *30
        ...
        # ( remove the word which the frequentcy is low )
        # 
    ...
    def getRandomContext(self, C=5):
        ...
        context = sent[max(0, wordID - C):wordID]  # Predecessor of word
        if wordID+1 < len(sent):
            context += sent[wordID+1:min(len(sent), wordID + C + 1)]  # Successor of word

        centerword = sent[wordID]  # center
        context = [w for w in context if w != centerword] #remove center word

        if len(context) > 0:
            return centerword, context
        else:
            return self.getRandomContext(C)  # unless all context are empty
    ...
    def rejectProb(self):
        # larger the freq is , larger the reject prob
